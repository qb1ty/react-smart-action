...
export const useOfflineMutation =
    <State, Payload, ServerResponse = unknown>(
        initialState: State,
        ...
        const processQueueAsLeader = useCallback(async () => {
                ...
                try {
                    ...
                    if (categorizedError.reason === "conflict") {
                        ...
                        const controls: QueueControls = {
                            removeTask: async (id) => { await removeTask(id); broadcastUpdate(); triggerSync() },
                            clearQueue: async () => { await clearQueue(optionsRef.current.mutationKey); broadcastUpdate() },
                            resumeSync: async () => triggerSync()
                        }
                    ...
                    const maxDelayMs = retryConfig.maxRetries || 30000

                    if (task.retryCount >= maxRetries) {
                        await removeTask(task.id)
                        
                        optionsRef.current.onRollback?.(categorizedError as any, initialState)
                        broadcastUpdate()
                        continue
                    }
            ...
            if (syncStatus !== "paused_network" && syncStatus !== "paused_conflict") {
                setSyncStatus("idle")
            }
        }, [initialState, syncStatus, broadcastUpdate])
